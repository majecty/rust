...
## 1. 워크스페이스
- members 9개: `toy/ast`, `toy/driver`, `toy/eval`, `toy/expand`, `toy/lexer`, `toy/mir`, `toy/resolve`, `toy/span`, `toy/tokenstream-lowering`
- 상위: `~/code/rust` workspace `resolver = "2"`
- 실행 스크립트: `toy/run` (+ `run.ts`)
- 파이프라인: 읽기 → lex → lowering → expand → resolve → eval (기본 `value:` 출력)
  - MIR 경로: resolve → `toy/mir` lowering → `--mir` 덤프 또는 `--mir-eval`(MIR 인터프리터)

## 2. crate별 상태
(검증: `cd toy && cargo test -p <crate> --lib`, 총 63개 통과)
...
- `rtoy-mir`: AST(resolve 후) → basic block CFG MIR — `Body{locals, arg_locals, entry, blocks}`, `BasicBlock{stmts, terminator}`, `Place{local, proj}`/`Rvalue`/`Operand`, `TerminatorKind{Goto/SwitchInt/Call/Return}`. `if`→`SwitchInt`, 최상위 `let`→prologue 블록, `dump()`(rustc `-Zunpretty=mir` 흉내) · 테스트 7
...
- `rtoy-driver`: `--lex`/`--ast`/`--mir`/`--mir-eval`/`--trace` + eval 배선(`Runtime::format_value`)
