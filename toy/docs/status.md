...
## 2. crate별 상태
(검증: `cd toy && cargo test -p <crate> --lib`, 총 79개 통과)
...
- `rtoy-thir`: THIR 데이터 + HIR → THIR lowering + **toy 첫 typeck** — arena `exprs`/`blocks`/`stmts`/`params`, `Ty::{I64,Unit,Struct,Infer}`, 반환 타입 fixpoint(재귀 호출), `ThirLowerError`(MismatchedTypes/NotAStruct/UnknownField/WrongArgCount), `dump()`(평탄 arena)/`dump_tree()`(`body`에서 재귀 전개, rustc thir-tree 대응) · 테스트 10
