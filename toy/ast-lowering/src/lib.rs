//! rtoy ast-lowering — AST(resolve 후)를 basic block CFG MIR로 내린다.
//! rustc의 `rustc_ast_lowering` 자리지만, 이 toy에는 HIR이 없어 곧바로 MIR로 간다.
//!
//! - `lower_crate`가 크레이트 전체(struct/fn 표 수집 + 함수별 lowering)를 맡는다.
//! - 함수 하나는 `FnLower`(rustc BlockAndBuilder 자리)가 블록을 채운다.
//! - `fn` 파라미터 AST가 없어 최상위 `let`을 "인자 자리"로 쓴다 (eval `bind_args` 관례).
//!   인자 n개로 호출되면 `bb n`부터 실행하면 되도록 prologue 블록을 앞에 깔아 둔다.

use rtoy_ast::{Block, Crate, Expr, ExprKind, FnItem, Ident, ItemKind, StmtKind as AstStmtKind};
use rtoy_mir::{
    BasicBlock, BlockId, Body, ConstValue, FieldDef, FnId, FnRef, LocalDecl, LocalId, MirCrate,
    Operand, Place, ProjectionElem, Rvalue, Stmt, StmtKind, StructDef, StructId, Terminator,
    TerminatorKind, Ty,
};
use rtoy_span::Span;
...
impl<'a> FnLower<'a> {
    ...
    /// 부수효과 없는 식(정수 리터럴·place)은 temp를 만들지 않고 그대로 참조한다.
    fn lower_operand(&mut self, expr: &Expr) -> Result<Operand, MirLowerError> {
        match &expr.kind {
            ExprKind::Int(n) => return Ok(Operand::Const(ConstValue::Int(*n))),
            ExprKind::Var { .. } | ExprKind::FieldAccess { .. } => return Ok(Operand::Copy(self.place_of(expr)?)),
            _ => {}
        }
...
mod tests {
    ...
    #[test]
    fn pure_operands_skip_temps() {
        // `a + b`의 두 항은 temp가 아니라 `_1`/`_2` 직접 참조여야 한다.
        let m = mir_of("fn main() { let a = 1; let b = 2; a + b }");
        let binop = m.fns[0]
            .blocks
            .iter()
            .flat_map(|b| &b.stmts)
            .find_map(|s| match &s.kind {
                StmtKind::Assign(_, Rvalue::BinaryOp(_, l, r)) => Some((l.clone(), r.clone())),
                _ => None,
            })
            .expect("BinaryOp 없음");
        let copy = |local| Operand::Copy(Place { local, proj: vec![] });
        assert_eq!(binop, (copy(1), copy(2)), "a/b는 temp 없이 직접 참조");
    }

    #[test]
    fn int_literal_stays_const_operand() {
        // `a + 2`의 리터럴은 temp(`x = const 2`)를 만들지 않는다.
        let m = mir_of("fn main() { let a = 1; a + 2 }");
        let body = &m.fns[0];
        assert_eq!(body.locals.len(), 2, "temp local 없이 `_0`+`_1(a)`만 있어야 함");
        let binop = body
            .blocks
            .iter()
            .flat_map(|b| &b.stmts)
            .find_map(|s| match &s.kind {
                StmtKind::Assign(_, Rvalue::BinaryOp(_, _, r)) => Some(r.clone()),
                _ => None,
            })
            .expect("BinaryOp 없음");
        assert_eq!(binop, Operand::Const(ConstValue::Int(2)));
    }
